...
- `GET /captive-login?email=<addr>` — submitted by the dashboard's "Continue" form; sets `captivePortalUnlocked = true` (see "Captive portal" below)
- `GET /hotspot-detect.html`, `/library/test/success.html` — Apple's captive-check probes
- `GET /generate_204`, `/gen_204` — Android's captive-check probes
- any other path — serves the dashboard (`server.onNotFound(handleRoot)`), so unrecognized captive-portal probe requests (routed here by the `DNSServer` in `setup()`, which resolves every hostname to the board's own IP) land on the real page instead of 404ing
...
That mini-browser is locked down (confirmed on iPhone: no tabs, no bookmarking, and critically, a `target="_blank"` link — the naive escape trick — does **nothing** in it; iOS blocks it from opening Safari). Two ways out on the dashboard instead:

- **Email-gated "Continue" form** (`/captive-login`), the real technique these portals use: a plain form GET (not fetch/XHR — normal in-page navigation works fine inside these restricted webviews, only *new tabs* are blocked), which just flips `captivePortalUnlocked = true` server-side. That alone doesn't dismiss anything — the OS polls its captive-check URL (`/hotspot-detect.html`, `/generate_204`, etc., matched by **path only**; the Host header the OS used, e.g. `captive.apple.com`, doesn't matter since DNS already routes it here) on its own timer in the background, and only *that* next probe — now answered with the literal response each OS expects from a real non-captive network (`handleCaptiveProbeApple`/`handleCaptiveProbeAndroid`) instead of the dashboard — is what makes it auto-close the window, typically within a few seconds. `onApStationConnected` resets `captivePortalUnlocked` to false whenever a *new* station joins the AP, so a second phone gets its own prompt rather than inheriting whatever the first one left behind (the flag is global, not per-client, since the ESP32 WebServer doesn't track that).
- **"Copy address" fallback** (Clipboard API, falling back to `execCommand("copy")`) for manually backing out of the captive window (its own Done/Cancel control) and pasting into Safari/Chrome.

If you add more platforms' captive-check paths (Windows' `/connecttest.txt`, `/ncsi.txt`; older Android's `/gen_204` variants), route them the same way: dashboard while locked, the platform's exact expected response once `captivePortalUnlocked`.
